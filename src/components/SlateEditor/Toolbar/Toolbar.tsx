...
export const Toolbar: React.FC<Props> = ({ editor }: Props) => {
  ...
  const toggleBlock = useCallback((editor: any, format: ElementType) => {
  ...
  return (
    <div className="flex flex-row justify-start items-start gap-x-5 h-[100px] p-2 w-full bg-white my-5 rounded-sm">
      <button onClick={() => handleMouseDownIcon('heading-one')}>
        <BsTypeH1 size={20} className={clsx(isBlockActive(editor, 'heading-one') ? 'text-black' : 'text-gray-400')} />
      </button>
      <button onClick={() => handleMouseDownIcon('heading-two')}>
        <BsTypeH2 size={20} className={clsx(isBlockActive(editor, 'heading-two') ? 'text-black' : 'text-gray-400')} />
      </button>
      <button onClick={() => handleMouseDownIcon('heading-three')}>
        <BsTypeH3 size={20} className={clsx(isBlockActive(editor, 'heading-three') ? 'text-black' : 'text-gray-400')} />
      </button>
    </div>
  )
}
